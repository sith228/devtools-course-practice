...
bool CreditCalculator::check_four_data(int amount, int period,
    double interest, int month_pas) {
        return (amount > 0 && period > 0 && period <= 60 && interest > 0 &&
            interest < 100 && month_pas >= 0 && month_pas <= period);
}

bool CreditCalculator::check_three_data(int amount, int period,
    double interest) {
...
double CreditCalculator::monthly_payment(int amount, int period,
    double interest) {
    if (!check_three_data(amount, period, interest))
        return -1;
...
double CreditCalculator::check_balance(int amount, int period,
    double interest, int month_pas) {
    if (!check_four_data(amount, period, interest, month_pas))
        return -1;
    CreditCalculator creditCalc;
    double paid_out;
