...
MatrixCalculator::MatrixCalculator(int rows = 0, int columns = 0) {
    ...
    matrix_ = std::vector<std::vector<double > >(rows, std::vector<double>(columns, 0.0));
}
...
MatrixCalculator MatrixCalculator::operator *(const MatrixCalculator& p) const {
    ...
    for (int i = 0; i < result.matrix_.size(); ++i) {
        for (int j = 0; j < result.matrix_[i].size(); ++j) {
            for(int k = 0; k < p.matrix_.size(); ++k) {
...
void MatrixCalculator::SetMatrix(std::vector<std::vector<double>> &input_matrix) {
